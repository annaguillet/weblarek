...
Поля: counterElement, basketButton.

Сеттер counter обновляет счетчик товаров.

Событие: basket:open при клике на корзину.

## Catalog

Список карточек товаров.

Поле: list — контейнер .gallery.

Сеттер products выводит карточки в DOM.

## CardBase

Общий родитель для всех карточек.

Хранит данные товара (_id, _title, _price, _inBasket).

## Сеттеры для DOM (title, price, inBasket).
...
Если price === null, кнопка блокируется («Недоступно»).
...
Общий родитель для форм (шаг 1 и 2).

Управляет валидацией, кнопками и ошибками.

## OrderForm

Форма выбора оплаты и адреса доставки.

Валидация payment и address.
...
Презентер — это слой, который связывает модели данных и представления. Он не хранит данные и не решает, как отображать элементы интерфейса. Его задача — обрабатывать события от моделей и представлений и запускать соответствующие действия.

## Основные принципы работы презентера:

Обработчик событий от представлений
Презентер подписывается на события, генерируемые компонентами представления (например, нажатие на кнопку "Купить", открытие корзины, выбор способа оплаты). После этого он вызывает методы моделей для изменения данных.

## Обновление интерфейса через события от моделей
После изменения данных модель должна эмитить событие update или подобное. Презентер подписан на эти события и вызывает метод render соответствующего компонента представления, передавая новые данные.

## Логика отображения модальных окон
Презентер следит за состоянием модальных окон (открыты/закрыты) и управляет их видимостью. Компоненты представления отвечают только за визуализацию, а не за решение, когда открывать или закрывать модальное окно.

## Обработка форм
Презентер проверяет данные, полученные из формы, с помощью методов моделей (Buyer.validateBuyerData) и, если ошибок нет, сохраняет данные в модели (Buyer.setBuyerData). После успешного сохранения данных презентер может инициировать следующие действия (например, открытие второго шага формы или отправку заказа на сервер).