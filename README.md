...
Компонент отображения корзины товаров и управления её содержимым.

Поля:

basketEl — основной контейнер корзины.

listEl — контейнер для списка товаров в корзине.

totalEl — элемент отображения общей суммы.

orderBtn — кнопка оформления заказа.

Конструктор:

events — объект для работы с событиями.

basket — модель корзины (Basket).

modal — модальное окно для отображения корзины.

Функции:

render() — отображает корзину и её содержимое.

renderContents(itemTemplate) — рендерит список товаров и обновляет сумму.

update() — обновляет содержимое корзины при изменении её данных.

getTemplates() — получает шаблоны корзины и элементов из DOM.

События:

basket:remove — удаление товара из корзины при клике на кнопку «удалить».

order:start — начало оформления заказа при клике на кнопку «Оформить».
...
Управляет последовательностью форм и модальных окон.
